  ...
  return (
    <div className="flex items-center gap-3">
      ...
      <span
        ...
        style={{
          color: isLoser ? 'var(--color-brand-text-muted)' : 'var(--color-brand-text)',
...
export function ToursPage({ matches, teams, loading, error }: ToursPageProps) {
  ...
  const selectedTeamA = selectedMatch ? teamMap.get(selectedMatch.team_a_id) : undefined
  const selectedTeamB = selectedMatch ? teamMap.get(selectedMatch.team_b_id) : undefined
  ...
  return (
    <>
            ...
            <div className="flex items-center gap-3 mb-4">
              <div
                ...
                style={{
                  ...
                  color: '#fff',
                  boxShadow: '0 0 10px rgba(0,117,49,0.35)',
                }}
              >
                {tourNum}
              </div>
              ...
              <p className="label-caps text-[10px] flex-shrink-0" style={{ color: 'var(--color-brand-text-muted)' }}>
                {tourMatches.length}{' '}
                {tourMatches.length === 1
                  ? 'матч'
                  : tourMatches.length < 5
                  ? 'матча'
                  : 'матчей'}
              </p>
            </div>
            ...
            <div className="space-y-3">
              {tourMatches.map(m => {
                ...
                return (
                  <div
                    key={m.id}
                    className="bento-card px-4 py-3.5 space-y-3 cursor-pointer transition-all duration-150 hover:brightness-110 active:scale-[0.98]"
                    onClick={() => setSelectedMatch(m)}
                    title="Нажмите для просмотра деталей матча"
                  >
                    ...
                    <div className="flex items-center gap-2" style={{ color: 'var(--color-brand-text-muted)' }}>
      ...
      {selectedMatch && selectedTeamA && selectedTeamB && (
        <MatchDetailModal
          match={selectedMatch}
          teamA={selectedTeamA}
          teamB={selectedTeamB}
