              ...
              >
                <div className="flex items-center justify-between gap-3">
                    ...
                    <div className="min-w-0">
                      ...
                      <div className="flex items-center gap-1 mt-0.5">
                        ...
                        <span className="text-xs" style={{ color: 'var(--color-brand-text-muted)' }}>
                          {team.players.length} {team.players.length === 1 ? 'игрок' : team.players.length < 5 ? 'игрока' : 'игроков'}
                        </span>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="flex items-center gap-2 flex-shrink-0">
                    <div className="w-1 h-10 rounded-full" style={{ backgroundColor: team.color ?? 'rgba(255,255,255,0.2)' }} />
                    {isAdmin && (
                      <button
                  ...
                  {team.players.length === 0 ? (
                    ...
                    <div className="px-2 py-3 space-y-1">
                      {team.players.map((p: any) => (
                        ...
                        >
                          {p.number != null
                            ? <div className="w-7 h-7 rounded-full flex items-center justify-center flex-shrink-0" style={{ background: 'rgba(255,255,255,0.10)' }}>
                                <span className="text-[11px] font-bold" style={{ color: 'var(--color-brand-text)' }}>{p.number}</span>
                              </div>
                          ...
                          {p.ban_matches > 0 && !p.permanent_ban && (
                            <span className="label-caps text-[9px] px-1.5 py-0.5 rounded" style={{ background: 'rgba(251,146,60,0.15)', color: '#fb923c' }}>
                              -{p.ban_matches}
                            </span>
                          )}
                          {p.permanent_ban && (
                            <span className="label-caps text-[9px] px-1.5 py-0.5 rounded" style={{ background: 'rgba(239,68,68,0.15)', color: '#f87171' }}>
                              БАН
                            </span>
                          )}
                        </div>
