...
const MEDAL: Record<number, { color: string; bg: string; border: string; label: string }> = {
  0: { color: '#FFB800', bg: 'rgba(255,184,0,0.08)',   border: 'rgba(255,184,0,0.25)',   label: '🥇' },
  1: { color: 'rgba(255,255,255,0.55)', bg: 'rgba(255,255,255,0.04)', border: 'rgba(255,255,255,0.15)', label: '🥈' },
  2: { color: 'var(--color-brand-primary)', bg: 'rgba(122,219,138,0.06)', border: 'rgba(122,219,138,0.18)', label: '🥉' },
}
          ...
          {filtered.map((s, i) => {
            ...
            return (
              <div
                ...
                onMouseLeave={e => {
                  (e.currentTarget as HTMLDivElement).style.background = medal ? medal.bg : i % 2 === 0 ? 'rgba(255,255,255,0.02)' : 'transparent'
                }}
              >
                {/* Position */}
                ...
                <div className="flex items-center gap-2 min-w-0 pr-2">
                  <div
                    ...
                    onClick={e => {
                      ...
                      e.stopPropagation()
                      setPhotoView({ src: s.photo_url, name: s.player_name })
                    }}
                    title={s.photo_url ? 'Нажмите для увеличения' : undefined}
                  >
                    {s.photo_url
                  ...
                  <div className="min-w-0">
                    ...
                    {(s.total_yellow > 0 || s.total_red > 0 || s.permanent_ban) && (
                      <div className="flex gap-1 mt-0.5">
                        {s.total_yellow > 0 && (
                          <span className="text-[10px] leading-none">🟨{s.total_yellow}</span>
                        )}
                        {s.total_red > 0 && (
                          <span className="text-[10px] leading-none">🟥{s.total_red}</span>
                        )}
                        {s.permanent_ban && (
